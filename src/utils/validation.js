    ...
    case 2:
      if (fieldName === "selectedProvinces") {
        return !value || value.length === 0
          ? "Veuillez sélectionner au moins une province d'opération"
          : null;
      }
      break;
    case 3:
  ...
  if (state.selectedProvinces && state.selectedProvinces.length > 0) {
    state.selectedProvinces.forEach((province) => {
      ...
      if (!localite || localite.trim() === "") {
        fieldErrors[
          `localite_${province.pro_id}`
        ] = `Veuillez renseigner les localités pour ${province.pro_designation}`;
      }
    });
  }
...
const validateIdentificationStep2 = (state) => {
  ...
  // Validation spéciale pour les domaines (tableau)
  const domaines = state.identificationData.domaines;
  if (!domaines || (Array.isArray(domaines) && domaines.length === 0)) {
    fieldErrors.domaines = "Veuillez sélectionner au moins un domaine d'intervention";
  }
  ...
  if (state.questionsAnswers.priseEnCharge === "oui") {
    ...
    if (!occasion || occasion.trim() === "") {
      fieldErrors.occasion =
        "Veuillez préciser à quelle occasion vous avez pris en charge les victimes";
    }
  }
